...
const convertStravaTypeToGinkgo = (
  type: string,
  sport_type: string,
...
export const fromStravaActivityToGinkgoActivity = (
  ...
  try {
    ...
    if (stravaActivity.streams?.time?.data) {
          ...
          geoBlock.heading =
            counter > 0 && stravaActivity.streams?.time?.data[counter + 1]
              ? MAP.calcBearing(
                  geoBlock.lat,
                  geoBlock.long,
                  (stravaActivity.streams?.latlng?.data[counter + 1] as Array<number>)[0] as number,
                  (stravaActivity.streams?.latlng?.data[counter + 1] as Array<number>)[1] as number,
                )
              : 0;
        }
        if (
          stravaActivity.streams?.velocity_smooth?.data &&
      ...
      if (activity.blocks.geoPositionBlocks.length > 0 && activity.blocks.heartBlocks.length > 0) {
        activity.metrics.metabolic = MP.calcAll(
          activity.blocks,
          activity.settings,
          activity.userInfo as ActivityUserInfo,
          activity.metrics,
          activity.type,
        ) as MetabolicMetrics;
      }
        ...
        stravaActivity.segment_efforts?.forEach((segmentEffort, index) => {
          ...
          for (const geoBlock of activity.blocks.geoPositionBlocks) {
            ...
            if (geoBlock.time >= startTime && geoBlock.time <= endTime) {
              ...
              const heartBlock: HeartBlock | undefined = activity.blocks?.heartBlocks?.find(
                (hrBlock) => hrBlock.time >= geoBlock.time - 250 && hrBlock.time <= geoBlock.time + 250,
              );
              ...
              const gyroBlock: GyroscopeBlock | undefined = activity.blocks?.gyroscopeBlocks?.find(
                (grBlock) => grBlock.time >= geoBlock.time - 250 && grBlock.time <= geoBlock.time + 250,
              );
              ...
              const accelBlock: AccellerationBlock | undefined = activity.blocks?.accelerationBlocks?.find(
                (accBlock) => accBlock.time >= geoBlock.time - 250 && accBlock.time <= geoBlock.time + 250,
              );
          ...
          const drillBlocks: ActivityBlocks = {
          ...
          const drill: Drill = {
            ...
            updatedAt: null,
            metrics: {
              gps: GPS.calcAll(drillBlocks, activity.settings) as GpsMetrics,
              heart: HR.calcValues(drillBlocks) as HeartMetrics,
              metabolic: MP.calcAll(
                drillBlocks,
                activity.settings,
                activity.userInfo as ActivityUserInfo,
                activity.metrics,
      ...
      if (!!stravaActivity.laps && stravaActivity.laps.length > 0) {
        activity.laps = stravaActivity.laps.map((lap, index) => {
          ...
          const lapBlocks: ActivityBlocks = {
            heartBlocks: drillHeartBlocks,
            geoPositionBlocks: drillGeoBlocks,
            accelerationBlocks: drillAccelBlocks,
            weatherBlocks: [],
          ...
          const ginkgoLap: Lap = {
            ...
            createdAt: null,
            metrics: {
              gps: GPS.calcAll(lapBlocks, activity.settings) as GpsMetrics,
              heart: HR.calcValues(lapBlocks) as HeartMetrics,
              metabolic: MP.calcAll(
                  lapBlocks,
                  activity.settings,
                  activity.userInfo as ActivityUserInfo,
...
export const fromStravaActivityToGinkgoActivityBase = (
  stravaActivity: StravaActivity,
  userInfo: {
    activityLevel: number;
    id: string;
