...
export const calcAll = (blocks: ActivityBlocks, settings: ActivitySettings, userInfo: ActivityUserInfo, metrics: ActivityMetrics, type: ActivityType): MetabolicMetrics => {
    let metabolicMetrics: MetabolicMetrics = {
    ...
    metabolicMetrics.idealWeight = userInfo.gender.toLowerCase() === 'male' ? Math.pow(userInfo.height, 2) + 22.1 : Math.pow(userInfo.height, 2) + 20.6;
    metabolicMetrics.basalMetabolism = userInfo.gender.toLowerCase() === 'male' ?
    ...
    metabolicMetrics.calorieConsumptionHr = metrics.heart?.avgHr ??
    userInfo.gender.toLowerCase() === 'male' ?
        ((-55.0969 + (0.6309 * metrics.heart.avgHr) + (0.1988 * userInfo.weight) + (0.2017 * userInfo.age)) / 4.184) * 60 * metrics.gps.totalTime :
        ((-20.4022 + (0.4472 * metrics.heart.avgHr) - (0.1263 * userInfo.weight) + (0.074 * userInfo.age)) / 4.184) * 60 * metrics.gps.totalTime;
...
const calcCalorieConumptionSpecific = (type: ActivityType, level: number, weight: number, totalDistance: number, avgpower: number, totalTime: number) => {
