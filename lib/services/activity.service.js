...
const initNewActivity = (type, name) => {
    return {
        ...
        reactions: [],
        privacyType: models_1.PrivacyTypeEnum.public,
    };
};
...
const convertDataFromGPX = (content, newActivity) => {
    ...
    const parsedGpx = GPX.default.parse(content);
    ...
    try {
        ...
        if (stravaActivity.streams?.time?.data) {
            for (let counter = 0; counter < stravaActivity.streams?.time?.data?.length; counter++) {
                ...
                geoBlock.time = new Date(stravaActivity.start_date).valueOf() + counter * 1000;
                ...
                if (stravaActivity.streams?.latlng?.data && stravaActivity.streams?.latlng?.data?.length > 0) {
                    ...
                    // @ts-ignore
                    geoBlock.heading =
                        counter > 0 && stravaActivity.streams?.time?.data[counter + 1]
                            ? index_1.MAP.calcBearing(geoBlock.lat, geoBlock.long, (stravaActivity.streams?.latlng?.data[counter + 1])[0], (stravaActivity.streams?.latlng?.data[counter + 1])[1])
                            : 0;
                }
                if (stravaActivity.streams?.velocity_smooth?.data &&
                    stravaActivity.streams?.velocity_smooth?.data?.length > 0) {
            ...
            if (!!stravaActivity.segment_efforts && stravaActivity.segment_efforts?.length > 0) {
                stravaActivity.segment_efforts?.forEach((segmentEffort, index) => {
                    let startTime = -1;
                    let endTime = -1;
                    const drillGeoBlocks = [];
                    const drillHeartBlocks = [];
                    const drillGyroBlocks = [];
                    const drillAccelBlocks = [];
                    for (const geoBlock of activity.blocks.geoPositionBlocks) {
                        if (geoBlock.lat === segmentEffort.segment.start_latlng[0] &&
                            geoBlock.long === segmentEffort.segment.start_latlng[1]) {
                            startTime = geoBlock.time;
                        }
                        if (geoBlock.lat === segmentEffort.segment.end_latlng[0] &&
                        ...
                        if (geoBlock.time >= startTime && geoBlock.time <= endTime) {
                            ...
                            const heartBlock = activity.blocks?.heartBlocks?.find((hrBlock) => hrBlock.time >= geoBlock.time - 250 && hrBlock.time <= geoBlock.time + 250);
                            if (!!heartBlock) {
                                drillHeartBlocks.push(heartBlock);
                            }
                            const gyroBlock = activity.blocks?.gyroscopeBlocks?.find((grBlock) => grBlock.time >= geoBlock.time - 250 && grBlock.time <= geoBlock.time + 250);
                            if (!!gyroBlock) {
                                drillGyroBlocks.push(gyroBlock);
                            }
                            const accelBlock = activity.blocks?.accelerationBlocks?.find((accBlock) => accBlock.time >= geoBlock.time - 250 && accBlock.time <= geoBlock.time + 250);
                            if (!!accelBlock) {
                                drillAccelBlocks.push(accelBlock);
                            }
                        }
                    }
                    const drillBlocks = {
                        heartBlocks: drillHeartBlocks,
                        geoPositionBlocks: drillGeoBlocks,
                        accelerationBlocks: drillAccelBlocks,
                        weatherBlocks: [],
                        gyroscopeBlocks: drillGyroBlocks,
                    };
                    const drill = {
                        id: '',
                        ...
                        updatedAt: null,
                        metrics: {
                            gps: index_1.GPS.calcAll(drillBlocks, activity.settings),
                            heart: index_1.HR.calcValues(drillBlocks),
                            metabolic: index_1.MP.calcAll(drillBlocks, activity.settings, activity.userInfo, activity.metrics, activity.type),
                        },
                    };
                    activity.drills?.push(drill);
                });
            }
            if (!!stravaActivity.laps && stravaActivity.laps.length > 0) {
                activity.laps = stravaActivity.laps.map((lap, index) => {
                    const drillGeoBlocks = [];
                    const drillHeartBlocks = [];
                    const drillGyroBlocks = [];
                    const drillAccelBlocks = [];
                    const startTime = lap.start_date.valueOf();
                    const endTime = lap.start_date.valueOf() + (lap.elapsed_time * 1000);
                    for (const geoBlock of activity.blocks.geoPositionBlocks) {
                        if (geoBlock.time >= startTime && geoBlock.time <= endTime) {
                            drillGeoBlocks.push(geoBlock);
                            const heartBlock = activity.blocks?.heartBlocks?.find((hrBlock) => hrBlock.time >= geoBlock.time - 250 && hrBlock.time <= geoBlock.time + 250);
                    ...
                    const ginkgoLap = {
                        id: '',
                        name: lap.name,
                        start: lap.start_date.valueOf(),
                        settings: activity.settings,
                        end: lap.start_date.valueOf() + (lap.elapsed_time * 1000),
                        updatedAt: null,
                        createdAt: null,
                        metrics: {
                            gps: index_1.GPS.calcAll(lapBlocks, activity.settings),
                            heart: index_1.HR.calcValues(lapBlocks),
                            metabolic: index_1.MP.calcAll(lapBlocks, activity.settings, activity.userInfo, activity.metrics, activity.type),
                        },
                    };
                    return ginkgoLap;
                });
