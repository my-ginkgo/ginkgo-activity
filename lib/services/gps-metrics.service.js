...
const calcValues = (blocks) => {
    ...
    console.log('CALC TOTAL TIME', blocks.geoPositionBlocks[blocks.geoPositionBlocks.length - 1].time, blocks.geoPositionBlocks[0].time, (0, models_1.getMillisecondsBetweenTwoDates)(blocks.geoPositionBlocks[blocks.geoPositionBlocks.length - 1].time, blocks.geoPositionBlocks[0].time));
    return gpsMetrics;
};
const calcRanges = (blocks, settings) => {
    ...
    for (let i = 1; blocks.geoPositionBlocks.length > i; i++) {
        const diffDistance = index_1.MAP.calcCrow(blocks.geoPositionBlocks[i - 1].lat, blocks.geoPositionBlocks[i - 1].long, blocks.geoPositionBlocks[i].lat, blocks.geoPositionBlocks[i].long);
