...
const utils_model_1 = require("../models/utils.model");
const calcAll = (blocks, settings, userInfo, type) => {
    return {
...
const calcValues = (blocks) => {
    ...
    gpsMetrics.totalTime = (0, utils_model_1.getMillisecondsBetweenTwoDates)(blocks.geoPositionBlocks[blocks.geoPositionBlocks.length - 1].time, blocks.geoPositionBlocks[0].time);
    return gpsMetrics;
};
...
const calcRanges = (blocks, settings) => {
    ...
    gpsMetrics.distanceRange = (0, utils_model_1.calculateValueInRange)(gpsMetrics.totalDistance / 1000, settings.geoPosition.distanceRange);
    return gpsMetrics;
};
